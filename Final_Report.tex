...
\section{Integrating the SMS app with our API}
	...
	\subsection{Implementation Details}
		\subsubsection{Sending}
			For sending encrypted SMS a new layout is needed, because the default SMS app is not capable of sending encrypted SMS. The layout is very simple. It consists out of a contact field, a text field and a send button. See figure \ref{fig:compose}.
			\begin{figure}[h]
				\centering
			...
			Finally the SMS will be split into multiple parts, if the text is longer than 160 characters and send to the receiver. At the same time, the unencrypted text will be stored in the conversation log for that contact.
		\subsubsection{Receiving}
			Once the user has assigned a key to a contact, decrypting SMS woks without any interference of the user. Basically, the steps of decrypting are reverse to encrypting. If a broadcast for a new incoming SMS is received, then:
			\begin{enumerate}
