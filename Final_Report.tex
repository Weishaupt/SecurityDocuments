...
\section{Integrating the SMS app with our API}
	\label{sec-sms-app}
	To prove the concept we developed is working, we had to implement at least one system app. System apps have the advantage, that they get delivered with the software and have a deeper integration into core Android.
	...
	\subsection{Considerations}
		...
		In this scenario, we assumed that the symmetric key would already be exchanged with the other party. Therefor we just had to handle sending and receiving SMS.
	...
	\subsection{Implementation Details}
		\subsubsection{Sending}
			...
			\texttt{content://sms}, so they can appear in the regular conversation history and receive status updates (for example a delivery notification). After encryption the SMS will be split in multiple parts, if necessary, and send using the Android API \texttt{SmsManager}.
